<?php
include("src/php-jwt/src/JWT.php");
...
class Token
{
    ...
    public function encrypt_decrypt($str, $action)
    {
        $key = 'my_openssl_KEY';
        $iv_key = 'my_iv_KEY';
        $method = "AES-256-CBC";
    ...
    public function encode_jwt($user)
    {   //กำหนด key สำหรับ encode jwt
        ...
        $jwt = $this->encrypt_decrypt($jwt, "encrypt");
    ...
    public function decode_jwt($jwt)
    {
        ...
        try {
            //ถอดรหัส token
            $jwt = $this->encrypt_decrypt($jwt, "decrypt");
